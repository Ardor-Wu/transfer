...
training_sets=("DB")
ms=(64)
gpus=(0 1 2 3)  # Updated to use GPU 0, 1, 2, and 3
encoder_losses=("0.04375" "0.0875" "0.175" "0.35")
    ...
    for training_set in "${training_sets[@]}"; do
        for m in "${ms[@]}"; do
            for encoder_loss in "${encoder_losses[@]}"; do
                # Determine GPU index
                gpu_index=$((job_counter % 4))  # Updated to use 4 GPUs (0, 1, 2, 3)
                device=${gpus[$gpu_index]}
                # Generate the name parameter
                name="${model_type}_${m}_${training_set}_encoder_loss_${encoder_loss//\//_}"
                # Create the logs directory path
                log_dir="logs/$name"
                # Command to create log directory and run the python script, redirecting all output
                cmd="mkdir -p \"$log_dir\" && python train_hidden.py new --epochs 400 --batch-size 12 --name \"$name\" --model_type \"$model_type\" --message \"$m\" --dataset \"$training_set\" --gpu \"$device\" --encoder_loss \"$encoder_loss\" > \"$log_dir/output.log\" 2>&1"
